    ...
    def get_hand_location(self, frame, hand_num=0, draw_landmark=True):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, landmark in enumerate(hand.landmark):
                ...
                # Debug visibility attribute
                has_visibility = hasattr(landmark, 'visibility')
                confidence = landmark.visibility if has_visibility else 1.0
                print(f"Landmark {id}: Has visibility = {has_visibility}, Confidence = {confidence}, Threshold = {self.adaptive_threshold}")
                # Re-enable threshold with lower value
                if confidence >= self.adaptive_threshold or not has_visibility:
...
def evaluate_hand_tracking(detector, dataset_path, split="train", num_samples=10):
    ...
    for img_id in img_ids:
        ...
        # Resize and get original dimensions for scaling
...
def main():
    detector = handDetector(detection_confidence=0.1, tracking_confidence=0.1)  # Lower the threshold
    ...
    baseline_error, baseline_fps = evaluate_hand_tracking(detector, dataset_path, split="train", num_samples=10)
